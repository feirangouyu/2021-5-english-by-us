...
(() => {
    ...
    //2.获取问题数据
    let getQuestionlData = function (data) {
        return new Promise((resolve, reject) => {
            $.ajax({
                url: "http://127.0.0.1:3000/getQuestionlData",
    ...
    let showData = function () {
        for (let i = 0; i < questionData.length; i++) {
            let time = questionData[i].ctime.split('T')[0];
            if (questionData[i].num == null) {
                questionData[i].num = 0
            }
            if (questionData[i].focus == null) {
                questionData[i].focus = 0
            }
            let url = "../reply/reply.html?user_id=" + rData.user_id + "&question_id=" + questionData[i].id
            let question = `
                <div class="question-item" id="${questionData[i].id}">
                <a href="${url}" target="_blank">
                    <h3 class="item-content">${questionData[i].question}</h3>
                </a>
                <div class="item-box">
                    <span class="btn-write">
                        <svg class="Zi Zi--Edit Button-zi" fill="currentColor" viewBox="0 0 24 24"
                    ...
                    <div class="item-status">
                        <span class="statusItem reply">${questionData[i].num} 回答</span>
                        <span class="statusItem focus">${questionData[i].focus} 关注</span>
                        <span class="statusItem time">${time} 发布</span>
                    </div>
                </div>
                </div>
            `
    ...
    async function init() {
        rData.user_id = getUrlParam()
        questionData = await getQuestionlData();
        showData();
        writeInit();
        ...
        $('.sumbit').on("click", function () {
            ...
            if (text.val() !== '') {
                ...
                let index = divClass.parent().attr('id')
                let i = $('.question-item').index($('#' + index))

                divClass.prev().children('.statusItem reply')
                data.num = questionData[i].num + 1
                data.id = parseInt(index)
                data.focus = questionData[i].focus
                rData.question_id = parseInt(index)

                divClass.prev().children('.item-status').children('.statusItem.reply').html(data.num + " 回答")
                setQuestionlData(data)
                setReplyData(rData)
            } else {
